...
const Card4 = ({ product }) => {
    ...
    const truncatedText = isExpanded ? description : description.substr(0, 150) + '...';
        ...
        <div className="p-2 sm:p-4 bg-white rounded-lg shadow-sm">
            <div>
                ...
                <div className='flex flex-col sm:flex-row gap-4 sm:gap-6 md:gap-10'>
                    ...
                    <div className='grid grid-cols-2 gap-2 sm:gap-4 md:gap-6'>
                        {product?.features?.map((feature) => (
                            <div key={feature?.id || Math.random()} className='flex items-center space-x-2'>
                                <img 
                                    src={feature?.url || ''} 
                                    alt={feature?.title || 'Feature'} 
                                    className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 object-contain" 
                                />
                                <p className="text-xs sm:text-sm md:text-base">{feature?.title || 'Unknown Feature'}</p>
                            </div>
                        )) || []}
                    </div>
                </div>
                ...
                <div className="mt-4 sm:mt-6">
                    <h1 className='text-lg sm:text-xl md:text-2xl font-bold mb-2'>{product?.title || 'No Title'}</h1>
